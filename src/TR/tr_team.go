...
var teams = [...]string{"Rainbow", "Blossom", "Turbo", "Pulse", "Lightning"}
...
func get_tr_urls(teamName string) (urls []string, extrNr int) {

	extrNr = 0
	html, err := get_team_status_html("radio_product_mhos", teamName)
	if err != nil {
		CheckAndExit(err, "get_team_status_html")
	}
	table := get_tr_per_team(teamName, html)
	targets := strings.Split(table, " target='_blank'>")
	re := regexp.MustCompile("https:.*\"")
	extrRe := regexp.MustCompile("<b>E W ")
